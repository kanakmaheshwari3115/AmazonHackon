...
//             <path d="M10 3.23V16.77L12.88 14.73L11.64 11.21L14.5 9.18H10.85L10 5.56V3.23Z" />
//         </svg>
//       )}
//       {[...Array(emptyStars)].map((_, i) => (
//         <svg key={`empty-${i}`} className="w-4 h-4 text-amazon-mediumGray" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.722c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
//       ))}
//       {reviewCount !== undefined && (
//         <span className="ml-1 text-xs text-amazon-linkBlue hover:underline cursor-pointer">{reviewCount}</span>
//       )}
//     </div>
//   );
// };

// const CoinIconSmall: React.FC = () => (
//   <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="h-3.5 w-3.5 mr-0.5 text-amazon-orange">
...
//     if (!showShippingChoice) {
//       onViewDetails(product);
//     }
//   };

//   const firstMaterial = product.materials && product.materials.length > 0 ? product.materials[0] : 'N/A';

//   return (
//     <div
//       onClick={handleCardClick}
//       className="bg-amazon-white dark:bg-amazon-navBlue rounded-md shadow-sm overflow-hidden transition-all duration-200 hover:shadow-lg border border-transparent dark:border-amazon-darkBlue hover:border-amazon-mediumGray/70 dark:hover:border-amazon-mediumGray/50 flex flex-col h-full w-full max-w-[250px] sm:max-w-[280px] cursor-pointer relative product-card-animate"
//     >
//       <div className="p-3 flex justify-center items-center h-48 sm:h-56"> {/* Fixed height for image container */}
//         <img className="max-w-full max-h-full object-contain" src={product.imageUrl} alt={product.name} />
//       </div>
//       <div className="p-3 flex flex-col flex-grow">
//         <h3
//             className="text-sm font-medium text-amazon-textBlack dark:text-amazon-white mb-1 leading-tight hover:text-amazon-orange dark:hover:text-amazon-orange transition-colors"
//             style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', textOverflow: 'ellipsis', height: '2.5em' }} // Approx 2 lines height
//             title={product.name}
//         >
//             {product.name}
//         </h3>

//         <StarRating score={product.ecoScore} reviewCount={Math.floor(product.ecoScore * 13 + 7)} />

//         <div className="text-xl font-semibold text-amazon-textBlack dark:text-amazon-white my-1">
...
//             <span className="inline-flex items-center text-xs font-medium text-green-700 dark:text-green-300 my-1" title={`Earn ${COINS_SUSTAINABLE_PURCHASE_HIGH_ECOSCORE} EcoCoins!`}>
//               <CoinIconSmall />
//               Earn {COINS_SUSTAINABLE_PURCHASE_HIGH_ECOSCORE} coins
//             </span>
//           )}

//         <p className="text-xs text-gray-500 dark:text-amazon-mediumGray mt-1 mb-2 hidden sm:block" style={{ display: '-webkit-box', WebkitLineClamp: 1, WebkitBoxOrient: 'vertical', overflow: 'hidden', textOverflow: 'ellipsis', height: '1.2em' }}>
//             {product.category} &bull; {firstMaterial}
//         </p>
...
//             className="w-full bg-amazon-yellow hover:bg-amber-400 text-amazon-textBlack font-medium py-1.5 px-3 rounded-lg shadow-sm hover:shadow focus:ring-2 focus:ring-amazon-orange focus:ring-offset-1 focus:outline-none transition duration-150 text-sm"
...
//       </div>
//       {showShippingChoice && (
//         <div className="absolute inset-0 bg-amazon-white/95 dark:bg-amazon-darkBlue/95 backdrop-blur-sm flex flex-col items-center justify-center p-4 z-10 rounded-md shadow-2xl">
//            <p className="text-base font-semibold mb-3 text-amazon-textBlack dark:text-amazon-white">Choose Shipping Option:</p>
//           <div className="flex items-center mb-3">
//              <p className="text-xs text-gray-600 dark:text-amazon-mediumGray mr-1">How it works?</p>
//             <ShippingOptionsPopover>
//                 <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4 text-amazon-linkBlue dark:text-sky-400 hover:opacity-80">
//                     <path strokeLinecap="round" strokeLinejoin="round" d="m11.25 11.25.041-.02a.75.75 0 0 1 1.063.852l-.708 2.836a.75.75 0 0 0 1.063.853l.041-.021M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Zm-9-3.75h.008v.008H12V8.25Z" />
//                 </svg>
//             </ShippingOptionsPopover>
//           </div>
//           <button
//             onClick={(e) => { e.stopPropagation(); handleSelectShipping(CartType.GROUP_BUY);}}
//             className="w-full bg-amazon-orange hover:opacity-90 text-white font-semibold py-2.5 px-4 rounded-lg mb-2.5 text-sm shadow-md"
//           >
//             Add to Group Buy Cart
//           </button>
//           <button
//             onClick={(e) => { e.stopPropagation(); handleSelectShipping(CartType.INDIVIDUAL);}}
//             className="w-full bg-amazon-linkBlue hover:opacity-90 text-white font-semibold py-2.5 px-4 rounded-lg text-sm shadow-md"
//           >
//             Add to Individual Cart
//           </button>
//           <button
//             onClick={(e) => { e.stopPropagation(); setShowShippingChoice(false);}}
//             className="mt-4 text-xs text-amazon-linkBlue dark:text-sky-400 hover:underline"
//           >
//             Cancel
//           </button>
//         </div>
