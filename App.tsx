...
//   const [geminiAvailableState, setGeminiAvailableState] = useState<boolean>(true); // Renamed to avoid conflict with imported function
...
//     if (achievementKey) {
//         setUserMilestones(prev => ({ ...prev, achievementsUnlocked: [...new Set([...prev.achievementsUnlocked, achievementKey])] })); // Ensure uniqueness
//     }
//   }, [addAlert]);
...
//   useEffect(() => { 
//     setGeminiAvailableState(isGeminiAvailable()); // Use imported function directly
//   }, []); // Run once on mount
...
//       return;
//     }
//     if (!geminiAvailableState) { // Use state variable
...
//     }
//     setShowQuizModal(null); // Close quiz modal
//   };
...
//     localStorage.setItem('isSellerRegistered', 'true'); // Persist registration status
...
//       estimatedCo2Saved: ESTIMATED_CO2_SAVING_PER_USED_ITEM_KG, // Example saving
//       images: listingData.images || ['https://picsum.photos/seed/default_list/400/300'], // Use previews or fallback
//     };
...
//     if (item.currency !== 'Trade') { // Only award coins if not a trade
...
//     setViewingMarketplaceListing(null); // Close detail modal
//   };

//   const handleOpenMarketplaceChat = (listing: MarketplaceListing) => {
//     setChattingWithSellerForListing(listing);
//     setCurrentMarketplaceChatMessages([]); // Reset messages for new chat
//     // Simulate initial greeting from seller
//     if(isGeminiAvailable()) { // Use imported function directly
...
//     // Simulate assessment based on reported condition (could be random too)
...
//     if (Math.random() < 0.1 && assessedCondition !== PackageCondition.HEAVILY_DAMAGED) { // 10% chance it's worse than reported (unless already reported as heavily damaged)
...
//         coinReward -= PENALTY_PACKAGE_SLIGHT_DAMAGE; // Apply penalty directly to base
//         coinReward = Math.max(0, coinReward); // Ensure coins don't go negative from this transaction
//         alertMessage = `Package ${packageId.slice(-6)} processed. Condition: Slightly Damaged. ${coinReward} EcoCoins awarded.`;
//     } else { // Heavily Damaged
//         finalStatus = ReturnPackageStatus.RETURN_REJECTED;
//         coinReward = 0; // No reward for heavily damaged
//         // Potentially a penalty could be applied by subtracting from userCoins, but for now, just no reward.
...
//             isGeminiAvailable={geminiAvailableState} // Use state variable
//             addAlert={addAlert}
...
//           />
//       </div>
//       {/* Placeholder for Amazon-style hero/banner */}
//       {/* <div className="h-64 bg-amazon-mediumGray rounded-md mb-6 flex items-center justify-center text-amazon-white text-2xl">Hero Banner / Carousel Placeholder</div> */}
...
//       />
//       {/* Example of another product section */}
//       <ProductList
//         products={products.slice(0,4).reverse()} // Just an example with different data
//         onViewDetails={handleViewDetails}
//         onAddToCart={handleAddToCart}
//         isLoading={isLoadingProducts}
//         title="Recently Viewed (Mock)"
//         triggerPurchaseAnimation={triggerPurchaseAnimation}
//       />
...
//       case 'analyzeExternal': return <ExternalProductAnalyzer addAlert={addAlert} isGeminiAvailable={geminiAvailableState} onAnalysisComplete={handleAnalysisComplete} />; // Use state variable
...
//         onNavigate={handleNavigation} // Pass handleNavigation here
...
//         {renderCurrentView()}
//       </main>
//       <SustainabilityChatbot analysisHistory={analysisHistory} isGeminiAvailable={geminiAvailableState} addAlert={addAlert}/> {/* Use state variable */}
//       <Footer 
...
//       {showCreateListingModal && <CreateListingModal isOpen={showCreateListingModal} onClose={() => setShowCreateListingModal(false)} onAddListing={handleAddMarketplaceListing} isGeminiAvailable={geminiAvailableState} addAlert={addAlert} />} {/* Use state variable */}
